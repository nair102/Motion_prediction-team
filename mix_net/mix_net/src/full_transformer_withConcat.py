...
class newMixNet(nn.Module):
    ...
    def __init__(self, params):
        ...
        Hist_layer = nn.TransformerEncoderLayer(
            d_model=2*params["encoder"]["hidden_size"],
        ...
        self._mix_out_layers = self._get_linear_stack(
            in_size=params["encoder"]["hidden_size"] * 6,
        ...
        self._vel_out_layers = self._get_linear_stack(
            in_size=params["encoder"]["hidden_size"]*2,
        ...
        self._dyn_embedder = nn.Linear(
            params["encoder"]["hidden_size"] * 6, params["acc_decoder"]["in_size"]
        )
    ...
    def forward(self, hist, left_bound, right_bound):
        ...
        hist_emb = self._concat_positional_encoding(hist_emb)
        left_emb = self._concat_positional_encoding(left_emb)
        right_emb = self._concat_positional_encoding(right_emb)
