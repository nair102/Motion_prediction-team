...
from mix_net.mix_net.src.full_transformer_withConcat import newMixNet
...
class MixNetTrainer:
    ...
    def __init__(self, params: dict, net: newMixNet):
    ...
    def train(self):
        ...
        with open ("train_metrices_withConcat.json","w")as f:
            json.dump(self.train_metrices,f)
    def visualize_metrics(self):
        epochs = self.train_metrices["epochs"]
        ...
        line1,=ax1.plot(epochs, self.train_metrices["train_path_loss"], label='Train Path MSE', marker='o')
        ax1.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
        for x,y in zip(epochs,self.train_metrices["train_path_loss"]):
            ax1.annotate(f'({x}, {y:.2f})', (x, y), textcoords="offset points", 
                xytext=(0, 10), ha='center', fontsize=8, color=line1.get_color())
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('MSE')
        ax1.set_title('Path Loss Across Epochs')
        ax1.legend()
        ax1.grid(True)
        ...
        line2,=ax2.plot(epochs, self.train_metrices["train_vel_loss"], label='Train Vel MSE', marker='o')
        ax2.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
        for x,y in zip(epochs,self.train_metrices["train_path_loss"]):
            ax2.annotate(f'({x}, {y:.2f})', (x, y), textcoords="offset points", 
                xytext=(0, 10), ha='center', fontsize=8, color=line2.get_color())
    ...
    def _prepare_model_dir(self, date_formatted, time_formatted):
        ...
        if self._params["training"]["model_save_path"] != "":
            ...
            file = os.path.join(
                os.path.dirname(self._model_save_path), "net_full_params.json"
            )
            with open(file, "w") as fp:
